...
namespace vcpkg::Build
{
    ...
    PreBuildInfo PreBuildInfo::from_triplet_file(const VcpkgPaths& paths, const Triplet& triplet)
    {
        ...
        pre_build_info.triplet_abi_tag = [&]() {
            ...
            auto hash = Commands::Hash::get_file_hash(fs, triplet_file_path, "SHA1");

            if (auto p = pre_build_info.external_toolchain_file.get())
            {
                hash += "-";
                hash += Commands::Hash::get_file_hash(fs, *p, "SHA1");
            }
            else if (pre_build_info.cmake_system_name == "Linux")
            {
                hash += "-";
                hash += Commands::Hash::get_file_hash(fs, paths.scripts / "toolchains" / "linux.cmake", "SHA1");
            }
            else if (pre_build_info.cmake_system_name == "Darwin")
            {
                hash += "-";
                hash += Commands::Hash::get_file_hash(fs, paths.scripts / "toolchains" / "osx.cmake", "SHA1");
            }
            else if (pre_build_info.cmake_system_name == "FreeBSD")
            {
                hash += "-";
                hash += Commands::Hash::get_file_hash(fs, paths.scripts / "toolchains" / "freebsd.cmake", "SHA1");
            }
            else if (pre_build_info.cmake_system_name == "Android")
            {
                hash += "-";
                hash += Commands::Hash::get_file_hash(fs, paths.scripts / "toolchains" / "android.cmake", "SHA1");
            }
